  ...
  $allowed_extension = array('pdf');
    ...
    $newname = $rename.'.'.$file_extension;
      ...
      $tempname = $_FILES["cv"]["tmp_name"];   
        $folder = "upload/cv/".$newname;
    
    $conectdb->query("INSERT into Information (fname, lname, email, cnumber, intro, cv) values ('$fname', '$lnam', '$eml', '$user', '$pass', '$newname')"); 
    if (move_uploaded_file($tempname, 'upload/cv/'.$newname))  {
            echo "<script>window.location='index.php?mess=Successfully Added'</script>";
        } 
  }else{
            echo "<script>window.location='index.php?mess=Not Successfully Added'</script>";
      }  
    }
?>
